...
ldd container_prog | awk '{ if($3 ~ /^\//) print $3 }' | xargs -I '{}' cp --parents '{}' $SIMPLE_CONTAINER_ROOT
...
unshare --pid --uts --fork chroot $SIMPLE_CONTAINER_ROOT /container_prog subtask2
...
mkdir -p /sys/fs/cgroup/simple_container
...
unshare --pid --uts --ipc --fork bash -c 'echo $$ > /sys/fs/cgroup/simple_container/cgroup.procs; exec chroot '"$SIMPLE_CONTAINER_ROOT"' /container_prog subtask2' &
CONTAINER_PID=$!
wait $CONTAINER_PID

# Remove the cgroup
rmdir /sys/fs/cgroup/simple_container
