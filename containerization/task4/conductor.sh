...
handle_run() {
    echo "DEBUG: Entering handle_run"
    echo "DEBUG: RUN command: '$1'"
    echo "DEBUG: Parent layers: '$2'"
    local command="$1"
    local parent_layers="$2"
    ...
    local temp_mount="$CACHEDIR/layers/$layer_hash/mnt"
    local temp_work="$CACHEDIR/layers/$layer_hash/work"
    mkdir -p "$temp_mount" "$temp_work"
    mount -t overlay overlay -o lowerdir="$parent_layers",upperdir="$CACHEDIR/layers/$layer_hash/diff",workdir="$temp_work" "$temp_mount"
...
# Record the metadata and parent layer for the new layer
handle_copy() {
    echo "DEBUG: Entering handle_copy"
    echo "DEBUG: COPY command arguments: '$1'"
    local args="$1"
    local parent_layers="$2"
    local parent_hash=$(get_layer_hash "$parent_layers")
    ...
    mkdir -p "$temp_mount/$dest"
    cp -r "$src"/* "$temp_mount/$dest/"
...
run() {
    ...
    # Instead of copying the filesystem, record the base/layer stack info.
    # Define paths
    CONTAINER_ROOTFS="$CONTAINERDIR/$NAME/rootfs"
    CONTAINER_DIFF="$CONTAINERDIR/$NAME/diff"
    CONTAINER_WORK="$CONTAINERDIR/$NAME/work"
    BASE_LAYER_STACK=$(cat "$IMAGEDIR/$IMAGE/layers")

    # Create required directories
    mkdir -p "$CONTAINER_ROOTFS" "$CONTAINER_DIFF" "$CONTAINER_WORK"

    # Mount the overlay filesystem
    mount -t overlay overlay -o lowerdir="$BASE_LAYER_STACK",upperdir="$CONTAINER_DIFF",workdir="$CONTAINER_WORK" "$CONTAINER_ROOTFS"
    ...
    # Create /dev inside the container rootfs if it doesn't exist
    mkdir -p "$CONTAINERDIR/$NAME/rootfs/dev"
    # Bind mount /dev into the container's /dev
    ...
    unshare --fork --pid --mount --ipc --net --uts --kill-child bash -c "exec chroot $CONTAINERDIR/$NAME/rootfs /bin/bash -c 'mount -t proc proc /proc && mount -t sysfs sysfs /sys && exec $INIT_CMD_ARGS'"
...
stop() {
    ...
    # Subtask 3.d.4
    # Unmount the overlay filesystem
    umount "$CONTAINERDIR/$NAME/rootfs" > /dev/null 2>&1 || :
...
exec() {
    echo "DEBUG: Entering exec"
    echo "DEBUG: Container name: '$1'"
    ...
    echo "DEBUG: Command to be executed: '$EXEC_CMD_ARGS'"
    ...
    # echo "DEBUG: Exact command executed: nsenter --target "$CONTAINER_INIT_PID" --mount --uts --ipc --net --pid chroot $ABS_PWD/$CONTAINERDIR/$NAME/rootfs $EXEC_CMD_ARGS"
    # nsenter --target $CONTAINER_INIT_PID --mount --uts --ipc --net --pid chroot $ABS_PWD/$CONTAINERDIR/$NAME/rootfs $EXEC_CMD_ARGS

    cmd="nsenter --target $CONTAINER_INIT_PID --mount --uts --ipc --net --pid chroot $ABS_PWD/$CONTAINERDIR/$NAME/rootfs $EXEC_CMD_ARGS"
    eval "$cmd" &
    sleep 1
}
