...
void kvm_run_vm(struct vm *vm1, struct vm *vm2)
{
    ...
    while (1)
    {
        ...
        case KVM_EXIT_IO:
            printf("VMFD: %d KVM_EXIT_IO\n", vm->vm_fd);
            //switch case for different ports
            switch (vm->vcpus->kvm_run->io.port)
            {
            case 16:
                prod_value[i] = *(int *)((char *)(vm->vcpus->kvm_run) + vm->vcpus->kvm_run->io.data_offset);
                printf("VMFD: %d Produced value: %d\n", vm->vm_fd, prod_value[i]);
                if(i == 2){
                    vm = vm2;
                    i = 0;
                }
                ...
                break;
            case 17:
                *(int *)((char *)(vm->vcpus->kvm_run) + vm->vcpus->kvm_run->io.data_offset) = prod_value[i];
