	...
	for (;;)
	{
		...
		case KVM_EXIT_IO:
			...
			if(vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT && vcpu->kvm_run->io.port == 0x29)
			{
				...
				printf("VMFD: %d Produced value: %d %d %d %d %d\n", vm->vm_fd, host_arr[0], host_arr[1], host_arr[2], host_arr[3], host_arr[4]);
				vm = vm2;
				vcpu = vcpu2;
				continue;
			}
			if(vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT && vcpu->kvm_run->io.port == 0x49)
			{
				...
				//convert guest virtual address to host virtual address using kvm_run->mmio.phys_addr
				int *host_arr = (int *)((char*)vm->mem + translate.physical_address);
				printf("VMFD: %d Consumed value: %d %d %d %d %d\n", vm->vm_fd, host_arr[0], host_arr[1], host_arr[2], host_arr[3], host_arr[4]);
				vm = vm1;
				vcpu = vcpu1;
				continue;
			}
			if(vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT && vcpu->kvm_run->io.port == 0x39)
			{
				uint32_t *p = (uint32_t *)((char *)vcpu->kvm_run + vcpu->kvm_run->io.data_offset);//read the value from the memory location
				//convert p to a pointer to a string
				struct kvm_translation translate;
